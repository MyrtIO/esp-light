...
static size_t light_hal_encode(rmt_encoder_t *encoder,
                               rmt_channel_handle_t channel,
                               const void *data,
                               size_t data_size,
                               rmt_encode_state_t *ret_state) {
    light_hal_strip_encoder_t *strip_encoder =
        __containerof(encoder, light_hal_strip_encoder_t, base);
    ...
    switch (strip_encoder->state) {
        case 0:
            encoded_symbols += strip_encoder->bytes_encoder->encode(
                strip_encoder->bytes_encoder, channel, data, data_size,
                &session_state);
...
static esp_err_t light_hal_transmit(void) {
    ...
    err = rmt_transmit(s_rmt_channel, s_rmt_encoder, raw_leds, sizeof(raw_leds),
                       &s_rmt_tx_config);
    if (err == ESP_OK) {
