...
static bool cpreferences_begin_read_or_create(
	cpreferences_t *prefs,
...
void persistent_data_load(persistent_data_t *data) {
	...
	if (!cpreferences_begin_read_or_create(prefs, NVS_NAMESPACE)) {
		ESP_LOGW(TAG, "failed to open namespace '%s', using defaults", NVS_NAMESPACE);
...
bool light_state_exists(void) {
	...
	if (!cpreferences_begin_read_or_create(prefs, NVS_LIGHT_STATE)) {
		ESP_LOGW(TAG, "failed to open namespace '%s' while checking light state", NVS_LIGHT_STATE);
		cpreferences_destroy(prefs);
		return false;
	}
...
void light_state_load(light_saved_state_t *state) {
	...
	cpreferences_t *prefs = cpreferences_create();
	if (prefs == NULL) {
		ESP_LOGW(TAG, "preferences create failed during light state load, using defaults");
		return;
	}

	if (!cpreferences_begin_read_or_create(prefs, NVS_LIGHT_STATE)) {
		ESP_LOGW(TAG, "failed to open namespace '%s', using default light state", NVS_LIGHT_STATE);
		cpreferences_destroy(prefs);
		return;
	}
...
void light_state_save(const light_saved_state_t *state) {
	...
	if (!cpreferences_begin(prefs, NVS_LIGHT_STATE, false)) {
		ESP_LOGW(TAG, "failed to open namespace '%s' for light state write", NVS_LIGHT_STATE);
		cpreferences_destroy(prefs);
		return;
	}
