...
static char effect_name_buf[32];
...
static void on_command(const uint8_t *payload, uint16_t length) {
	...
	JsonDocument &doc = HomeAssistant::json;
	...
	if (!doc["effect"].isNull() && ha_state.effect != nullptr) {
		strncpy(effect_name_buf, ha_state.effect, sizeof(effect_name_buf) - 1);
		effect_name_buf[sizeof(effect_name_buf) - 1] = '\0';
		light_cmd_t cmd;
		cmd.type = LIGHT_CMD_EFFECT;
		cmd.effect_name = effect_name_buf;
		light_send_cmd(&cmd);
	}
	...
	if (!doc["color"].isNull()) {
		...
		light_send_cmd(&cmd);
	} else if (!doc["color_temp"].isNull() && ha_state.colorTemp != 0) {
		light_cmd_t cmd;
