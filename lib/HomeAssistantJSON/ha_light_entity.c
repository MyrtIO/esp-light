...
bool ha_light_serialize_config(
	...
	lwjson_serializer_add_string(&ser, LK("name"),
		entity->name, strlen(entity->name));

	const char *id = ha_entity_id(entity);
	lwjson_serializer_add_string(&ser, LK("unique_id"), id, strlen(id));

	if (entity->icon != NULL) {
		lwjson_serializer_add_string(&ser, LK("icon"),
			entity->icon, strlen(entity->icon));
	}
	...
	if (cmd != NULL) {
		lwjson_serializer_add_string(&ser, LK("command_topic"),
			cmd, strlen(cmd));
	}
	...
	lwjson_serializer_add_string(&ser, NULL, 0,
		entity->device->id, strlen(entity->device->id));
	...
	if (config->effect_count > 0) {
		...
		for (uint16_t i = 0; i < config->effect_count; i++) {
			lwjson_serializer_add_string(&ser, NULL, 0,
				config->effects[i], strlen(config->effects[i]));
		}
		lwjson_serializer_end_array(&ser);
	}
...
bool ha_light_serialize_state(
	...
	if (state->effect != NULL) {
		lwjson_serializer_add_string(&ser, LK("effect"),
			state->effect, strlen(state->effect));
	}
	lwjson_serializer_add_uint(&ser, LK("brightness"), state->brightness);
	lwjson_serializer_add_string(&ser, LK("state"),
...
uint8_t ha_light_parse_command(
	...
	if (t != NULL && t->type == LWJSON_TYPE_STRING) {
		...
		if (fx != NULL && fx_len > 0) {
			size_t n = fx_len < sizeof(effect_buf) - 1
				? fx_len : sizeof(effect_buf) - 1;
